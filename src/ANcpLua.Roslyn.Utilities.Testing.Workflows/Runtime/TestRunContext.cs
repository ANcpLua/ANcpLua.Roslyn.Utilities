...
/// <summary>
/// Full IRunnerContext impl backed by in-memory dictionaries. Use ConfigureExecutor
/// to register an Executor under its id, then BindWorkflowContext to get a per-executor
/// IWorkflowContext that captures sent messages and yielded outputs into Queue&lt;&gt; instances.
/// </summary>
public class TestRunContext : IRunnerContext
{
    ...
    private sealed class BoundContext(string executorId, TestRunContext runnerContext, IReadOnlyDictionary<string, string>? traceContext) : IWorkflowContext
    {
        ...
        public ValueTask<T?> ReadStateAsync<T>(string key, string? scopeName = null, CancellationToken cancellationToken = default) => new(default(T?));

        public ValueTask<HashSet<string>> ReadStateKeysAsync(string? scopeName = null, CancellationToken cancellationToken = default) => new([]);
    ...
    ValueTask<StepContext> IRunnerContext.AdvanceAsync(CancellationToken cancellationToken) => throw new NotImplementedException();
