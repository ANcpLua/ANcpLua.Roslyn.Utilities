...
public static class TrimAssert
{
    ...
    ///     Thrown indirectly via Environment.Exit(-1) if the type still exists at
    ///     runtime.
    /// </exception>
    /// <remarks>
    ///     This method uses reflection to verify that a type has been trimmed away during the trimming process.
    ///     If the type is unexpectedly found at runtime, an error is written to the error stream and the process exits with
    ///     code -1.
    /// </remarks>
    [UnconditionalSuppressMessage("Trimming", "IL2057",
        Justification = "Intentional runtime type lookup to verify trimming behavior")]
    public static void TypeTrimmed(string typeName, string assemblyName)
    {
        ...
        if (type != null)
        {
            Console.Error.WriteLine(
                $"FAIL: Type '{typeName}' from assembly '{assemblyName}' was expected to be trimmed away but was found at runtime.");
            Environment.Exit(-1);
        }
    }
    ...
    /// <param name="assemblyName">The name of the assembly containing the type (e.g., "MyAssembly").</param>
    /// <exception cref="InvalidOperationException">
    ///     Thrown indirectly via Environment.Exit(-2) if the type was unexpectedly
    ///     trimmed.
    /// </exception>
    /// <remarks>
    ///     This method uses reflection to verify that a type has been preserved during the trimming process.
    ///     If the type is unexpectedly trimmed and not found at runtime, an error is written to the error stream and the
    ...
    public static void TypePreserved(string typeName, string assemblyName)
    {
        ...
        {
            Console.Error.WriteLine(
                $"FAIL: Type '{typeName}' from assembly '{assemblyName}' was expected to be preserved but was trimmed away.");
            Environment.Exit(-2);
        }
    }
}