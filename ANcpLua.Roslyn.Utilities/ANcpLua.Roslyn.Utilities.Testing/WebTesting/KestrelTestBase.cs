    ...
    where TProgram : class
{
    ...
    {
        _baseFactory = factory.WithWebHostBuilder(builder => { builder.ConfigureTestServices(ConfigureTestServices); });
    }

    /// <summary>HTTP client for making requests to the test server. Available after <see cref="InitializeAsync" />.</summary>
    protected HttpClient Client =>
        _client ?? throw new InvalidOperationException("Server not started. Call InitializeAsync first.");
    ...
    protected FakeLogCollector Logs =>
        _logs ?? throw new InvalidOperationException("Server not started. Call InitializeAsync first.");
