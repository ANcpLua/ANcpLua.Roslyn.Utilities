...
internal static class DeclarationChainExtractor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return DiagnosticFlow.Ok(chain.Count == 0 ? default : chain.ToArray().ToEquatableArray());
    }

    private static TypeDeclarationModel BuildModel(TypeDeclarationSyntax declaration)
    {
        var modifiers = declaration.Modifiers.Select(modifier => modifier.ValueText).ToList();
        if (!modifiers.Any(modifier => string.Equals(modifier, "partial", StringComparison.Ordinal)))
        ...
        return new TypeDeclarationModel(
            ...
            ConstraintClauses: constraints.Length == 0 ? default : constraints.ToEquatableArray());
    }
}
