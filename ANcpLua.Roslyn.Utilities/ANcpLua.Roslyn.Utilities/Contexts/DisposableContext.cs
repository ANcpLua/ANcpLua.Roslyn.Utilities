...
/// <remarks>
/// <list type="bullet">
/// <item><description>Caches symbols for <see cref="System.IDisposable"/>, <c>IAsyncDisposable</c>, and common disposable types.</description></item>
/// <item><description>Provides type classification methods for streams, database types, synchronization primitives, and more.</description></item>
/// <item><description>Includes static helpers for detecting <c>using</c> statements and dispose methods.</description></item>
...
sealed class DisposableContext
{
    ...
    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Runtime.InteropServices.SafeHandle"/>, or <c>null</c> if not available.
    /// </summary>
    ...
    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Data.Common.DbConnection"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? DbConnection { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Data.Common.DbCommand"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? DbCommand { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Data.Common.DbDataReader"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? DbDataReader { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Net.Http.HttpClient"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? HttpClient { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Net.Http.HttpMessageHandler"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? HttpMessageHandler { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.CancellationTokenSource"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? CancellationTokenSource { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.Timer"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? Timer { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.Semaphore"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? Semaphore { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.SemaphoreSlim"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? SemaphoreSlim { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.Mutex"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? Mutex { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.ReaderWriterLock"/>, or <c>null</c> if not available.
    /// </summary>
    public INamedTypeSymbol? ReaderWriterLock { get; }

    /// <summary>
    /// Gets the <see cref="INamedTypeSymbol"/> for <see cref="System.Threading.ReaderWriterLockSlim"/>, or <c>null</c> if not available.
    /// </summary>
    ...
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> has a <c>void Dispose()</c> method with no parameters; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <param name="type">The type symbol to check.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> has a <c>DisposeAsync()</c> method with no parameters; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <summary>
    /// Determines whether the specified operation is contained within a <c>using</c> statement or <c>using</c> declaration.
    /// </summary>
    /// <param name="operation">The operation to check.</param>
    /// <returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from <see cref="System.IO.Stream"/>; otherwise, <c>false</c>.
    /// </returns>
    public bool IsStream(ITypeSymbol? type) =>
        type is not null && Stream is not null && type.IsOrInheritsFrom(Stream);

    /// <summary>
    /// Determines whether the specified type is or inherits from <see cref="System.IO.TextReader"/> or <see cref="System.IO.TextWriter"/>.
    /// </summary>
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from <see cref="System.Data.Common.DbConnection"/>; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from <see cref="System.Data.Common.DbCommand"/>; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from <see cref="System.Data.Common.DbDataReader"/>; otherwise, <c>false</c>.
    /// </returns>
    /// <seealso cref="IsDbConnection"/>
    /// <seealso cref="IsDbCommand"/>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from <see cref="System.Net.Http.HttpClient"/>; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from a synchronization primitive type; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> equals <see cref="System.Threading.CancellationTokenSource"/>; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <param name="type">The type symbol to check, or <c>null</c>.</param>
    /// <returns>
    /// <c>true</c> if <paramref name="type"/> is or inherits from <see cref="System.Runtime.InteropServices.SafeHandle"/>; otherwise, <c>false</c>.
    /// </returns>
